...
if (isset($_POST['caridata'])) {
	$_SESSION['session_cari'] = $_POST['cari'];
	$keyword=$_SESSION['session_cari'];
}else{
	$keyword=$_SESSION['session_cari'];
}
                  ...
                  <table class="table align-items-center table-flush" id="table" >
                    ...
                    <tbody>
                    	<?php
								...
								$nomer = 1;
								$data= mysqli_query($conn,"SELECT * FROM rapat WHERE status != 'Selesai' AND judul LIKE '%$keyword%' OR status LIKE '%$keyword%' ORDER BY tglrapat ASC");
								while($t=mysqli_fetch_array($data)){
									...
									echo"<td>$nomer</td>";
									echo"<td>$t[judul]</td>";
									?><td><?php echo date("l, d F Y ", strtotime($t['tglrapat']))?></td><?php
									echo"<td>$t[jamrapat]</td>";
									if ($t['status']=='Segera') {
